...
router.post('/signin', async (req, res, next) => {
    ...
    try {
        ...
        res.cookie('access_token', token, { httpOnly: true,secure:true,SameSite:'Strict',path:'/' }).status(200).json({msg :Valid,Token:token});
...
router.post('/google', async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            res.cookie('access_token', token, { httpOnly: false }).status(200).json(user);
        }
        else {
            ...
            res.cookie('access_token', token, { httpOnly: false }).status(200).json(newUser);
        }
    } catch (error) {
        console.log(error)
        next(error)
